...
class ExperienceBuffer:
    """
    This class provides an expirience buffer.
    ...
    def __init__(self, is_new, capacity, seq_len, height, width):
        ...
        self.states = np.memmap("states.npy", dtype=np.uint8, mode=mode, shape=(2, capacity, seq_len, height, width)) #grayscale viewport no preprocessing    
        self.next_states = np.memmap("next_states.npy", dtype=np.uint8, mode=mode, shape=(2, capacity, seq_len, height, width)) #grayscale viewport no preprocessing
        self.actions = np.memmap("actions.npy", dtype=np.uint8, mode=mode, shape=(capacity)) #thats enough for actions
        self.rewards = np.memmap("rewards.npy", dtype=np.float32, mode=mode, shape=(capacity)) #тут могут быть дробные значения
    ...
    def append(self, state, next_state, action, reward, is_done):
        ...
        ----------
        state : numpy array
            numpy array of the following shape [2, seq_len, height, width].
        ...
        """
        #state is list of [np.array shape of [seq_len, height, width], np.array shape of [seq_len, height, width]] The same for next_state
        if self.cur_index == self.capacity:
            self.cur_index = 0
            self.is_full = True
        self.states[0, self.cur_index] = state[0] #left viewport
        self.states[1, self.cur_index] = state[1] #middle viewport
        self.next_states[0, self.cur_index] = next_state[0] #left viewport
        self.next_states[1, self.cur_index] = next_state[1] #middle viewport
    ...
    def sample(self, batch_size):
        ...
        -------
        states : tuple
            tuple of two numpy arrays of the following shape[batch_size, seq_len, height, width]. 
            Each for a viewport.
        next_states : tuple
            tuple of two numpy arrays of the following shape[batch_size, seq_len, height, width]. 
            Each for a viewport.
        ...
        states = (self.states[0, indexes], self.states[1, indexes])
        next_states = (self.next_states[0, indexes], self.next_states[1, indexes])
